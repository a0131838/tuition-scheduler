...
export default function RenewalWorkbenchClient({ initialTasks }: { initialTasks: RenewalTask[] }) {
  ...
  async function load(status = filter) {
    setFilter(status);
    const response = await fetch(`/api/admin/renewals?status=${encodeURIComponent(status)}&limit=300`, { cache: "no-store" });
    const data = await response.json();
    if (!response.ok || !data.ok) throw new Error(data.message || "加载失败");
    setTasks(data.tasks || []);
    setExpanded((current) => current || data.tasks?.[0]?.id || "");
  }
  ...
  async function scan() {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/admin/renewals", { method: "POST" });
      const data = await response.json();
  ...
  async function save(row: RenewalTask, statusOverride?: string) {
    ...
    if (status === "PARENT_NOTIFIED" && !draft.evidenceUrl && !row.evidenceUrl) {
      setMessage("确认已提醒家长前，请先上传微信群发送截图。");
      return;
    }
    setBusyId(row.id);
    setMessage("");
  ...
  async function copy(text: string | null) {
    ...
    setMessage("家长沟通文案已复制，请核对后发送到原微信群。");
  }
                ...
                <div className={styles.detail}>
                  ...
                  <div className={styles.columns}>
                    <div className={styles.workflow}>
                      ...
                      <label>家长微信群
                        <input value={String(draft.parentWechatGroupName || "")} onChange={(event) => setDraft(row.id, "parentWechatGroupName", event.target.value)} placeholder="填写实际群名" />
                      </label>
                      <label>家长回复
                        <textarea value={String(draft.parentResponse || "")} onChange={(event) => setDraft(row.id, "parentResponse", event.target.value)} placeholder="记录家长原意、顾虑和下一步" />
                      </label>
                      <label>内部备注
                    ...
                    <div className={styles.communication}>
                      ...
                      <button onClick={() => copy(row.parentMessage)}>复制文案</button>
                      ...
                      <div className={styles.quickActions}>
                        <button disabled={!row.evidenceUrl} onClick={() => save(row, "PARENT_NOTIFIED")}>确认已提醒家长</button>
