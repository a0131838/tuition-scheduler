...
async function createTemplate(teacherId: string, formData: FormData) {
  ...
  if (!Number.isFinite(weekday) || weekday < 0 || weekday > 6 || startMin == null || durationMin <= 0) {
...
export default async function TeacherDetailPage({
  ...
  if (!teacher) {
    return (
      <div>
        <h2>{t(lang, "Teacher Not Found", "老师不存�?)}</h2>
        <a href="/admin/teachers">�?{t(lang, "Back", "返回")}</a>
      </div>
    );
  }

  const [subjects, levels, students, campuses, rooms, templates] = await Promise.all([
      ...
      <form action={updateTeacher.bind(null, teacherId)} style={{ display: "grid", gap: 8, marginBottom: 16, maxWidth: 860 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <input
            ...
            placeholder={t(lang, "Years Experience", "教学经验(�?")}
            style={{ width: 220 }}
          />

          <select name="teachingLanguage" defaultValue={teacher.teachingLanguage ?? ""}>
      ...
      <form action={createTemplate.bind(null, teacherId)} style={{ display: "grid", gap: 8, maxWidth: 900 }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <select name="roomId" defaultValue="">
            <option value="">{t(lang, "Room (optional)", "教室(可�?")}</option>
            {rooms.map((r) => (
      ...
      {plan && (
        <div style={{ marginTop: 12 }}>
          <div style={{ marginBottom: 8 }}>
            ...
            {t(lang, "Conflicts", "冲突")} {plan.conflicts.length}. {t(lang, "Will create", "将创�?)} {plan.toCreate.length}.
          </div>
