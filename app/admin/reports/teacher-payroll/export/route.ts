...
export async function GET(req: Request) {
  ...
  const teacherProfiles = await prisma.teacher.findMany({
    ...
    select: {
      ...
      paymentProfileVerifiedAt: true,
      paymentProfileVerifiedBy: true,
      bankName: true,
  ...
  const header = [
    ...
    choose(lang, "Wise Account Holder", "Wise 户名"),
    choose(lang, "Wise Email", "Wise 邮箱"),
    choose(lang, "Wise Phone", "Wise 手机号"),
    choose(lang, "WiseTag", "WiseTag"),
    choose(lang, "Wise Country", "Wise 国家"),
    choose(lang, "Wise Currency", "Wise 币种"),
    choose(lang, "Payment Profile Status", "收款资料核验状态"),
    choose(lang, "Verified At", "核验时间"),
    choose(lang, "Verified By", "核验人"),
    choose(lang, "Legacy Bank Name", "历史银行名称"),
    choose(lang, "Legacy Bank Account Name", "历史银行户名"),
    choose(lang, "Legacy Bank Account Number", "历史银行账号"),
    choose(lang, "Legacy SWIFT / Branch Code", "历史 SWIFT / 分行代码"),
    choose(lang, "Sessions", "课次数"),
    choose(lang, "Cancelled+Charged", "取消但计薪"),
    choose(lang, "Included in Monthly Salary", "已含在全职月薪"),
  ...
  for (const row of rows) {
    ...
    lines.push(
      [
        ...
        profile?.payNowValue ?? "",
        profile?.payNowName ?? "",
        profile?.wiseAccountName ?? "",
        profile?.wiseEmail ?? "",
        profile?.wisePhone ?? "",
        profile?.wiseTag ?? "",
        profile?.wiseCountry ?? "",
        profile?.wiseCurrency ?? "",
        formatPaymentProfileStatus(profile?.paymentProfileStatus),
        profile?.paymentProfileVerifiedAt?.toISOString?.() ?? "",
        profile?.paymentProfileVerifiedBy ?? "",
        profile?.bankName ?? "",
