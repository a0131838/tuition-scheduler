...
export default function TeacherLeadEmailAddClient({
  labels,
}: {
  labels: { email: string; noteOptional: string; addLead: string; errorPrefix: string };
}) {
    ...
    <div style={{ display: "grid", gap: 6, marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "end" }}>
        <label style={{ display: "grid", gap: 4 }}>
          <span style={{ fontSize: 12 }}>{labels.email}</span>
        ...
        <button
          ...
          onClick={async () => {
            ...
            setBusy(true);
            try {
              const res = await fetch("/api/admin/manager/teacher-leads", {
                method: "POST",
        ...
        >
          {busy ? `${labels.addLead}...` : labels.addLead}
        </button>
      </div>
    </div>
  );
