...
export default function UserObserverModeClient({
  userId,
  enabled,
  labels,
}: {
  userId: string;
  enabled: boolean;
  labels: { observer: string; save: string; confirm: string; errorPrefix: string };
}) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/admin/manager/users/${userId}/observer`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ isObserver: checked }),
      });
