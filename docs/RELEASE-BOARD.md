...
- Current release line on this branch: `2026-06-01-r169` (Business Accounts invoices now reserve the same global `RGT-yyyymm-xxxx` sequence as student/New Oriental finance documents, and Business Accounts receipts use the same receipt number pattern and orange PDF template style), intended for the next production deploy from this branch.
