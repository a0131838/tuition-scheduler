...
- Post-deploy verification:
  - production is running feature commit `78e945d65afe4a747ac6a1814737656558bcabbf` with PM2 online and zero restarts;
  - `/admin/login` returns HTTP `200`, the deployed filter is the native GET implementation, and the focused regression covers changed, repeated-identical and empty submissions.
...
- Production verification:
  - feature commit 9a479bcb1fbdf3dd2d5e09df4fe15a9108106995 matched local, GitHub and server; all 129 migrations were current, PM2 PID 2997687 was online and /admin/login returned HTTP 200;
  - post-deploy production read-only projection for the affected session returned zero remaining reminders, confirming the nine historical students are absent.
...
- Business impact:
  - the main report no longer leads with confidence labels, evidence counts, long skill tables or repeated disclaimers;
  - every new report carries a three-stage improvement plan derived from its actual weakest domains;
  - historical stored reports receive the same display structure without recalculation or database rewrites;
  - technical evidence remains available under an explicit collapsed detail control;
  - unmeasured abilities are labelled as unmeasured instead of appearing as zero in the main result;
  - consultation opens with the student's assessment summary and priority domains already filled in.
- Verification before deploy:
...
- Current release line: `2026-08-07-r352` is live at runtime feature commit `53f83e9fee7156751cb2ff3f6b39d16680e77e73`. It fixes the Mini Program rank-zero bug, shows all international-school filters in one horizontal rail and routes four private/alternative/support programmes into detailed private-specialist profiles. WeChat development version `1.0.48` was uploaded successfully.
...
- Deployment: runtime feature commit `53f83e9fee7156751cb2ff3f6b39d16680e77e73`, PM2 PID `2201294`, health HTTP 200. Live catalog has 56 international brands and the exact five-school top order. WeChat development version `1.0.48` uploaded at 723,740 bytes.
...
- Deployment: runtime feature commit `471eb67038da61b1e0f843cf0192aa43bf5e6e6c`, PM2 PID `2182840`, health HTTP 200. Live catalog has 60 unique browse ranks and the exact five-school top order. WeChat development version `1.0.47` uploaded at 724,213 bytes.

## 2026-08-07-r350 Live

- Scope: first-tier consumer label, private/PEI route correction and result-first school details.
- Business impact:
  - exactly SAS, Dulwich, UWCSEA, Tanglin and NLCS display “第一梯队” without changing recommendation scoring;
  - DIMENSIONS, Insworld, Stalford, 5 Steps, SISH and GUILD now live under private/specialist profiles;
  - HWA and Olympiad remain in international schools because they publish complete IB or Cambridge school paths;
  - Web and Mini Program school details default to academic results and university outcomes.
- Safety boundary: public guide only; no authenticated operation, database or business workflow changed.
- Verification before deploy: 66 school-guide tests, 174 backend regressions, TypeScript, native Mini Program JavaScript syntax, 67-page Mini Program audit, 243-page production build and 390×844 browser checks passed.
- Post-deploy verification: catalog `2026-08-07-r350`, 60 brands split 23/17/5/15, exactly five first-tier schools, no migrated PEI/private route in the international list, six destination profiles present and directory/detail HTTP 200.
- Rollback point: `0a03d199b18268ac5079437ad58931a447f7b2d1`.
- Deployment: runtime feature commit `aff83c10a2e68390d4806138e55b618d6d6e5e7f`, PM2 PID `2165580`, health HTTP 200. WeChat development version `1.0.46` uploaded at 724,146 bytes.

## 2026-08-07-r349 Live
...
- Deployment: runtime feature commit `c2d8cf68bf7ebf2099fe1114037803419f0fc41b`, PM2 PID `2141755`, health HTTP 200 and live catalog split 23/22/5/16 with no visible tier-label leak. WeChat development version `1.0.45` uploaded at 724,024 bytes.
...
- Deployment: runtime feature commit `6a413de5976af799ea994cba438886ec2feec5e4`, PM2 PID `2098705`, health HTTP 200 and live catalog `2026-08-07-r348` with 66 international-school brands in the expected order. WeChat development version `1.0.44` uploaded at 724,161 bytes.
...
- Business impact:
  - the five Polytechnics and six Autonomous Universities now appear exactly once as individual institutions;
  - Anglo-Chinese School (Independent), Hwa Chong Institution, Raffles Institution and the other mixed secondary-JC institutions are included in the complete 19-school 2026 JAE JC/MI route;
  - NUS High, SOTA, Singapore Sports School and School of Science and Technology remain in the government specialist-school route instead of being treated as ordinary JCs;
...
- Scope: detailed public Poly, Autonomous University, BCA Academy and preschool directory.
- Business impact:
  - parents can open each of the five Polytechnics and six Autonomous Universities separately instead of reading one aggregate card;
  - every university profile shows QS 2027 context, 2025 GES outcomes, salary median, China-student application steps, programme positioning and fee/Tuition Grant cautions;
  - every Poly profile shows its own curriculum focus and DAE route, while employment remains clearly labelled as the official five-school joint result;
  - BCA Academy is a separate specialist category and its website employment claim is not mixed with MOE GES;
  - preschool now includes a China-family decision guide and separate profiles for PCF Sparkletots, My First Skool, MY World, Skool4Kidz and E-Bridge.
- Files: `lib/school-guide-public-postsecondary.ts`, `lib/school-guide-preschool-profiles.ts`, institution directory, Mini Program cache version and focused tests.
- Verification: 51 focused school-guide tests, 174 configured backend regression tests, 67-page Mini Program audit, JavaScript syntax, TypeScript and 243-page production build passed.
- Deployment: runtime feature commit `66da4335bfd4c636c9beb6f3d2dcd66a4e72d5ee`, PM2 PID `2022960`, health HTTP 200, live catalog `2026-08-07-r344`, WeChat development version `1.0.41` uploaded at 716,495 bytes. Experience-version designation and physical-phone content acceptance remain manual.
- Rollback point: `57b9c52f6f8f0c0302b95c463ca57b9093182241`.

## 2026-08-07-r343 Live

- Scope: compact public school-guide home hero and stronger popular-school recognition.
- Business impact:
  - the title and search now begin immediately below the native navigation bar instead of being pushed down by a fixed 430rpx hero;
  - the hero uses one compact brand mark, headline and search surface with no decorative grid or empty upper field;
  - popular schools use readable acronym identity marks and distinct restrained colours, with an image slot ready for authorised locally stored crests;
  - no unlicensed school crest is copied, and parent/staff portals plus all authenticated workflows remain unchanged.
- Files:
...
- Scope: public Mini Program school-guide readability and information hierarchy.
- Business impact:
  - the first screen now presents one plain-language promise, search and four parent tasks instead of a long explanation stack;
  - popular schools use horizontal browsing and the home no longer repeats full English names or audience descriptions;
  - punctuation-like Unicode navigation glyphs are removed;
  - school facts, admissions detail, private-higher-education partner programmes and pathway rules remain complete but start collapsed;
  - update dates, next review dates, source notes and assessment boundaries move to visually secondary page footers;
  - parent/staff portals and all authenticated operating workflows remain unchanged.
- Files:
  - `miniapp/boss-academic-parent/pages/guide-home/`
  ...
  - `tests/miniapp-consumer-entry.test.ts`
- Verification: 37 focused tests, all 174 backend regression tests, TypeScript, native JavaScript/JSON checks, the 67-page Mini Program audit, WeChat developer-tool preview compile and the 243-page production build passed.
- Deployment: local/GitHub/server runtime commit `5152e85533a8424b39477791b0a329f749d4edb2`, PM2 PID `1832298`, health HTTP 200, WeChat development version `1.0.39` uploaded at 714,352 bytes. Physical-phone typography and horizontal-scroll acceptance remain manual.
- Rollback point: `9ec8ca9f5ad3f9f77da3b748c21a12a0c4948b76`.

## 2026-08-06-r341 Live
...
- Rollout result: all 28 PDF pages passed visual inspection; extracted text confirmed 19-23 English items per pack with no mathematics section. 32 focused tests, 174 backend tests, TypeScript, native checks, the 67-page Mini Program audit and 243-page build passed. Runtime feature commit `95152f2a8f6bc4b404c91b952198d770bea71df0` aligned locally, on GitHub and on the server with PM2 PID `1791300` and HTTP 200. Live catalog returned `r341`, all four PDF endpoints passed, and WeChat development version `1.0.38` uploaded at 712,253 bytes.
- Rollback point: `fef606a2e6c9e0da8dd5bd0de6269fad80949599`.

## 2026-08-06-r340 Live

- Scope: direct free family assessment plus optional consultant analysis, and evidence-aware `冲刺 / 匹配 / 相对稳妥 / 过渡` school recommendations.
- Safety boundary: public school-guide assessment and recommendation only; legacy code/review history remains readable; no operational-system changes.
- Rollout result: 20 focused tests, 174 backend tests, TypeScript, native Mini Program checks, the 67-page audit and 243-page build passed. Runtime feature commit `f11b025d2736357b2550f56c8c8b8f69db412690` aligned locally, on GitHub and on the server; PM2 PID `1777580` was online and health returned HTTP 200. Live API returned version `2026-08-06-r340` and all four recommendation bands. WeChat development version `1.0.37` uploaded successfully at 712,253 bytes.
- Rollback point: final aligned `2026-08-06-r339` head `50e6c307b3f62d12505eb7720bf1f4fa0ea93f8d`.
...
- Business impact:
  - all ten priority providers now separate direct campuses, self-awarded degrees and external awarding partners;
  - each partner card lists its own undergraduate, postgraduate, doctorate or pathway programme names instead of combining every partner into one generic paragraph;
  - each partner displays QS World University Rankings 2027 context, and explicitly says when the overall ranking is not applicable or the institution is not listed;
  - current teach-out or not-yet-approved offerings are not presented as normal open programmes;
  - Web and Mini Program use the same structured source data and visual hierarchy;
...
- Rollout result: runtime feature commit `2fd3518146ad870b88c4c8bef333b8795805765e` is aligned locally, on GitHub and on the production server; PM2 PID `1742381` is online and `/admin/login` returns 200. Live API checks confirmed the expected catalog, subgroup counts and detail profiles. WeChat development version `1.0.35` uploaded at 710,808 bytes; set it as the experience version before physical-phone acceptance.
- Task doc: `docs/tasks/TASK-20260806-school-guide-consumer-structure-and-private-he.md`.
...
- Launch gate: a new DRAFT project needs the signed Full Care agreement, parent report access, start date, scope, owner, reviewer and initial plan before ACTIVE.
...
- Parent/public Mini Program:
  ...
  - a guardian-approved assessment code starts an age/path-specific A, B or C form and saves every answer for resume on the same device;
  - six anchor items select an easy, standard or hard route; DSA uses its configured task route;
  - the public session payload never returns answer keys, explanations or scoring rules.
- Staff Mini Program:
  - ADMIN, Academic/CS and SALES can issue one-use, 14-day codes; ADMIN and TEACHER can review open tasks;
  - staff can see pending/completed sessions, code status and assessment operation logs;
  - review correction creates a separate audit action instead of silently rewriting history.
- Safety boundaries:
  - additive assessment tables only; no existing table or business record is deleted or rewritten;
  - all assessment writes are recorded in `AuditLog`;
...
- Post-deploy verification:
  - implementation commit `569f3890` is on GitHub and the production server; login health is HTTP 200
  - protected staff AI API remains 401 anonymously
  - clean native WeChat development version `1.0.52` uploaded successfully, `754,946` bytes
