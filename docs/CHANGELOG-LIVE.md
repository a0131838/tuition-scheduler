...
- Date/Time (Asia/Singapore): 2026-08-19
- Deployment status: LIVE at runtime commit `78e945d65afe4a747ac6a1814737656558bcabbf`; server-only Ticket Center filter lifecycle correction, with no WeChat package change.
...
- Verification: 6/6 focused navigation regressions, 180/180 backend regressions, the 251-page production build, guarded deployment, PM2 online with zero restarts, deployed-source inspection and `/admin/login` HTTP `200` passed.
...
- Date/Time (Asia/Singapore): 2026-08-18
- Deployment status: LIVE; WeChat development version 1.0.60 uploaded, with no review submission or formal publication.
...
- Verification: focused report/communication tests 9/9 and backend tests 180/180 passed; TypeScript, the 251-page production build, the 69-page miniapp release audit and git diff --check passed. Guarded deployment completed for feature commit 41b682aa1efd9673dd2a78cc4700cf9d29739b62 with all 129 migrations current, PM2 PID 2971673 online and /admin/login HTTP 200. The protected miniapp PDF endpoint returned 401 without authentication. WeChat development version 1.0.60 uploaded at 796,307 bytes.
...
- Verification:
  ...
  - serialized deployment completed at application commit `ae267e21245b8587ecd40748cfb4c6ab7cd97a8b`; PM2 PID `2631632` is online and `/admin/login` returned HTTP 200
  - deployed candidate helper confirmed Allie at 32/50 subject hours (64%) and Steven at 26/50 subject hours (52%) within their shared package
...
- Native Mini Program development version `1.0.53` uploaded successfully from the same audited clean release worktree with AppID `wxe7017f8545e8ad49`. WeChat DevTools confirmed `代码上传成功`; the existing experience version was overwritten after the explicit platform warning. Formal public release still requires the normal WeChat review/release step.
...
- Verification: 69 school-guide tests, 174 backend regressions, TypeScript, native Mini Program JavaScript syntax, the 67-page Mini Program audit, the 243-page production build and a 390×844 browser check passed. Live catalog `2026-08-07-r352` returns 56 international brands with Tanglin first; all four migrated detail APIs return HTTP 200. Runtime feature commit `53f83e9fee7156751cb2ff3f6b39d16680e77e73` ran under PM2 PID `2201294`. WeChat development version `1.0.48` uploaded at 723,740 bytes.
...
- Verification: 68 school-guide tests, 174 backend regressions, TypeScript, native Mini Program JavaScript syntax, the 67-page Mini Program audit, the 243-page production build and 390×844 browser checks passed. Live catalog `2026-08-07-r351` returns 60 unique browse ranks, the agreed five-school top order and expected IB/British/American/other leaders. Production list/detail checks contain no generic Student’s Pass fallback, all three routes return HTTP 200 and runtime feature commit `471eb67038da61b1e0f843cf0192aa43bf5e6e6c` ran under PM2 PID `2182840`. WeChat development version `1.0.47` uploaded at 724,213 bytes.
...
- Key files:
  - `lib/school-guide-public-postsecondary.ts`
  - `lib/school-guide-preschool-profiles.ts`
  - `lib/school-guide-official-institutions.ts`
  - `lib/school-guide-directory.ts`
  - `tests/school-guide-public-postsecondary.test.ts`
...
- Key files:
  - `app/api/public/school-guide/academic-assessment/start/route.ts`
  - `app/api/public/school-guide/assessment/route.ts`
  - `lib/school-guide-match.ts`
  - `app/school-guide/assessment/AssessmentForm.tsx`
  - `miniapp/boss-academic-parent/pages/guide-academic-assessment/`
  - `miniapp/boss-academic-parent/pages/guide-assessment/`
...
- Formal implementation commit `569f3890` was deployed to `sgtmanage.com` and returned HTTP 200; the protected staff AI API returned 401 anonymously as expected.
- Clean native WeChat development version `1.0.52` uploaded successfully for AppID `wxe7017f8545e8ad49`; package size `754,946` bytes.
...
- 网页工单首页改为与员工小程序一致的动作优先队列：每张工单只显示当前需要处理的一步、截止时间和负责人。
- 工单详情固定为“家长需求 → 当前决定 → 按需展开正式执行 → 完整资料与历史”，不再让教务在长页面中寻找入口。
- 已在别处处理、确实无需处理和部分已处理改为详情顶部的一次性核验；历史工单没有原课程时无需伪造课程记录。
- 工作地图、超时队列、录入链接、完整筛选器和旧宽表格保留在高级区域，不影响管理和审计。
- 正式排课、改课、取消、换老师仍走原权限、原课程、老师时间、冲突、课包和幂等校验；无数据库迁移，无真实课程或财务数据改写。
- 专项测试 16/16、全量回归 558/558、251 页生产构建、标准服务器发布和线上健康验收通过。

# 2026-08-19-r384（后台读取性能第二阶段已上线）

- 移除后台共享导航每次切页都重算完整审批汇总的阻塞读取；审批中心和审批内容保持原数据源，侧栏只不再实时显示数量。
- 同一次页面请求中的员工会话按会话令牌复用，避免布局、语言和业务页面重复查询相同登录身份；不跨请求缓存权限，也不改变退出登录和账号权限。
- 签到告警页面先读取已有告警并返回，60 天同步改在响应后执行；月课表、工单详情和今日工作台的相互独立读取改为并行。
- AI 工单仍使用原 2.5 秒超时、人工入口和正式系统校验；排课、工单、课包、课时、考勤、工资、财务及通知规则没有改变。
- 验证：后台完整回归 180/180、性能结构回归 6/6、Next.js 生产构建和差异检查通过；标准服务器发布及线上登录态页面复测通过。正式环境完整加载耗时：工单中心 721ms、今日工作台 1119ms、签到告警 732ms、月课表 760ms、工单详情 810ms。

# 2026-08-19-r385（工单筛选状态修复已上线）

- 工单中心主筛选改为由真实页面导航状态控制“正在筛选”，请求完成后按钮会恢复，不再依赖网址必须发生变化才结束加载状态。
- 空条件筛选会携带明确的“本次由用户应用”标记，不再被上次记忆的负责人、类型或状态覆盖；清空和重复应用相同条件也能正确完成。
- 仅调整工单列表的筛选交互与状态传递，不修改工单数据、筛选含义、排课执行、AI 判断或其他后台页面功能。
- 验证：后台完整回归 180/180、性能结构回归 6/6、251 页生产构建及差异检查通过；已完成标准服务器发布。正式环境工单页完整加载约 1.3–1.4 秒，空条件和重复应用同一筛选均能完成，按钮会恢复为“筛选”。
