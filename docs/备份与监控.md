...
- COS 有时无法通过 AWS CLI 设置 Lifecycle（可能要求 Content-MD5）。本项目默认用“脚本清理”来控制对象存储保留期（见下文 2.4）。
...
对象存储默认不会自动删除旧备份。为避免备份无限增长，本项目在每次上传后会尝试清理旧对象：
- 脚本：`ops/server/scripts/cleanup_object_storage_backups.sh`
- 默认保留：30 天（`S3_RETENTION_DAYS=30`）
- 可关闭：`S3_CLEANUP_ENABLED=false`

在服务器上配置（可选）：

```bash
sudo tee -a /etc/tuition-scheduler/backup.env >/dev/null <<'EOF'
# Object storage retention (days)
S3_RETENTION_DAYS=30
S3_CLEANUP_ENABLED=true
# DRY_RUN=true  # Optional: only print deletions
EOF
sudo chown ubuntu:ubuntu /etc/tuition-scheduler/backup.env
sudo chmod 600 /etc/tuition-scheduler/backup.env
```
