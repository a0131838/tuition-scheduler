...
REPORT_SCRIPT="$REPO_ROOT/scripts/audit-upload-integrity.ts"
...
if [[ "$exit_code" -ne 0 ]]; then
  msg="Tuition Scheduler upload integrity job FAILED on $(hostname) at $(date -Iseconds)"
  msg+=$'\n'"exitCode=$exit_code"
  if [[ -n "${ALERT_WEBHOOK_URL:-}" ]]; then
    send_webhook_alert "$ALERT_WEBHOOK_URL" "$msg"
  fi
  if [[ -n "${ALERT_EMAIL_TO:-}" ]] && command -v mail >/dev/null 2>&1; then
    echo -e "$msg" | mail -s "Upload integrity job FAILED: $(hostname)" "$ALERT_EMAIL_TO" || true
  fi
  exit "$exit_code"
fi

read -r total_missing total_checked detail_path summary_path < <(
  python3 - "$OUT_FILE" <<'PY'
import json
import pathlib
import re
...
try:
    ...
    raise SystemExit(0)
print(
    int(obj.get("totalMissing", 0)),
    int(obj.get("totalChecked", 0)),
...
if [[ "${total_missing:-0}" -gt 0 ]]; then
  msg="Tuition Scheduler upload integrity ALERT on $(hostname) at $(date -Iseconds)"
  ...
  if [[ -n "${ALERT_EMAIL_TO:-}" ]] && command -v mail >/dev/null 2>&1; then
    echo -e "$msg" | mail -s "Upload integrity ALERT: ${total_missing} missing files" "$ALERT_EMAIL_TO" || true
    sent_any=1
  fi
  if [[ "$sent_any" -eq 1 ]]; then
    echo "$LOG_PREFIX alert sent (missing=$total_missing)"
  else
    echo "$LOG_PREFIX missing uploads detected (missing=$total_missing), but no alert channel configured"
  fi
else
  echo "$LOG_PREFIX ok (missing=0, checked=$total_checked)"
fi
