...
LOG_FILE="${LOG_FILE:-$LOG_DIR/tuition-scheduler_renewal_followup.log}"
CRON_TZ_VALUE="${CRON_TZ_VALUE:-Asia/Singapore}"
CRON_EXPR="${CRON_EXPR:-0 8 * * *}"
MARKER="# tuition-scheduler-renewal-followup"

mkdir -p "$LOG_DIR"

ENTRY="$CRON_EXPR mkdir -p '$LOG_DIR' && cd '$APP_DIR' && /usr/bin/npm run renewals:sync >> '$LOG_FILE' 2>&1 $MARKER"
...
echo "Renewal follow-up cron installed:"
echo "CRON_TZ=$CRON_TZ_VALUE"
echo "$ENTRY"
