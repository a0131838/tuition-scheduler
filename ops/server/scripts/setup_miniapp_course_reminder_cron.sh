...
mkdir -p "$LOG_DIR"
ENTRY="*/5 * * * * mkdir -p '$LOG_DIR' && cd '$APP_DIR' && /usr/bin/npm run miniapp:queue-course-reminders >> '$QUEUE_LOG' 2>&1 && /usr/bin/npm run miniapp:send-course-reminders >> '$SEND_LOG' 2>&1 $MARKER"
