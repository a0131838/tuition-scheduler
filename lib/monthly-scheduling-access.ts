...
export function monthlySchedulingAccessFor(input: { role: string; workspaces: string[]; manager: boolean }) {
  const canManage = input.role === "ADMIN" || input.role === "CS" || input.workspaces.includes("CS") || input.manager;
  return {
...
export async function requireMonthlySchedulingUser() {
  ...
  const access = monthlySchedulingAccessFor({ role: user.role, workspaces: user.workspaces, manager });
