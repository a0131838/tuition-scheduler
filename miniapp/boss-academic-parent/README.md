...
- 默认首屏是家长登录，只保留一个家长微信登录主操作。
- 员工通过页面底部“进入员工端”进入独立员工登录页。
- 家长邀请码和员工绑定码不在首屏并列；微信登录判定未绑定后会自动进入对应绑定页。
- 小程序会记住上次使用的家长端或员工端；双重身份用户可在学生列表或员工首页底部切换。
